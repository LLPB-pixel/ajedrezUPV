            ...
            for(int i = 0; i <8; i++){
                for(int j = 0; j <8; j++){
                    switch(tablero.tablero[i][j]){
                        ...
                        case 'B':
                            legalMoves += getBishopLegalMoves(color, i, j, tablero);
                        case 'N':
                            legalMoves += getKnightLegalMoves(color, i, j, tablero);
                        case 'Q':
                            legalMoves += getKnightLegalMoves(color, i, j, tablero);
                        case 'K':
                            legalMoves += getQueenLegalMoves(color, i, j, tablero);
                        case 'R':
                            legalMoves += getRookLegalMoves(color, i, j, tablero);
                    }
                }
            }
        }
                ...
                for(int j = 0; j <8; j++){
                    switch(tablero.tablero[i][j]){
                        ...
                        case 'q':
                            legalMoves += getQueenLegalMoves(color, i, j, tablero);
                        case 'k':
                            legalMoves += getKingLegalMoves(color, i, j, tablero);
                        case 'r':
                            legalMoves += getRookLegalMoves(color, i, j, tablero);
...
void fenToBoard(const std::string& fen, char board[8][8]) {
    int row = 0, col = 0;

    for (char c : fen) {
        if (c == '/') {
            row++;
            col = 0;  // Pasamos a la siguiente fila
        } else if (isdigit(c)) {
            // Si es un número, llenamos espacios vacíos con '.'
            int emptySquares = c - '0';
            for (int i = 0; i < emptySquares; ++i) {
                board[row][col++] = '.';
            }
        } else if (isalpha(c)) {
            // Si es una letra, es una pieza de ajedrez
            board[row][col++] = c;
        } else {
            break;  // Fin de la sección del tablero en el FEN
        }
    }
}
